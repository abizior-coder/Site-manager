...
| entries | `openAdd`, `submitAdd`, `newEntry`/`addEntry`, `openEditEntry`, `openEditTime`/`saveEditTime`, `deleteEntryFn`/`confirmDelete`/`restoreEntry`/`purgeEntry`, `approveEntry`, `toggleBreak`, `startDayOn`, `clockOut`, `submitNote`, voice input `toggleVoiceInput` (`continuous`/`interimResults`, so a pause between sentences does not end the dictation — only final segments are appended, once, `[data-voice-note]`) |
| reports (Rapport) | `generateDayReport` (scope via `dayReportScope`, manager-only whole-crew toggle `[data-report-whole-crew]`), `saveReportAsPdf`/`generateProjectsReport` (lazy `report-document.js`), `printRapport`, `sendReportToSupervisor`, `resendReport`, `openRapport`/`saveRapport`, week/day figures from `reports.js`. `reportViewModal`'s editing controls (notes, the pre-model hours override, exclude/restore, Save, Resend — `[data-report-actions]`) are `canManage()`-gated; viewing (figures, entry rows, PDF) stays open to anyone who can already reach the modal, from either the top-level tab or a job's own Rapporte tab (`docs/specs/2026-09-11_job-rapporte-tab.md`) |
...
| customers / contacts | `openCustomerForm`, `submitCustomer`, `deleteCustomer` (two-step), `submitContact`, `dueFollowUps`, import via `customers-import.js` (`stageCustomersFile`, `applyCustomersImport`). `customerForm` carries the full field set of `docs/specs/2026-09-09_customer-fields.md` (`anrede`, `uidChe` shown only while `company` is set, `role`, `phoneMobile`/`phoneOffice`, `lang`, `objectAddress`/`billingAddress`, `preferredChannel`, a repeating `contactPersons[]`, `source`) alongside the untouched legacy fields and `contacts[]` activity log; the detail view's Call/WhatsApp/Route tap row and `printDocument`'s billing address all read through `customer-contact.js`'s `customerContact()` rather than the old fields directly |
...
| AI (Worker `/`) | `callClaude`, `translateEntry`/`translateNote`/`autoTranslateNote`, `runScan`/`confirmScan` (delivery notes), library scans, `runInspection`, `openDayScan` (opens `dayScanModal`; the call itself and the save happen inside the lazy `ui/day-scan-modal.jsx`, `docs/specs/2026-09-14_ai-day-scan.md`) |
...
| `tabs/ProjectDetail.jsx` | `ProjectDetail`, `PhotoViewer`, `PhotoEditor` | the job hub `[data-hub-tabs]` (chat, files, material, inspections, trips), trash `[data-deleted-block]`. Each `[data-job-trip-row]` is tappable (`onEditTrip`, opens `ui/trip-modal.jsx` pre-filled) — delete stays a separate button, not nested inside it. Distinct from the top-level Transport tab's own `[data-trip-row]` list in `roofing-site-manager.jsx` (unrelated, unchanged by `docs/specs/2026-09-09_transport-detail-and-scan.md` — that spec named the job view only). The Fotos hub tab's `[data-photo-add]` (`docs/specs/2026-09-11_photo-add-in-category.md`) calls the app's existing `onAdd("photo")` directly, in both the empty state (`EmptyState`'s `action`) and next to the filled grid's header — no detour through the app-level quick-add sheet or chat, no new save path. The Rapporte hub tab (`hubTab === "reports"`) lists this job's own `sentReports` (`[data-job-report-row]`, `docs/specs/2026-09-11_job-rapporte-tab.md`) above the signed customer-Rapport list, computing each row's title/hours (`reportRows`/`reportTotals` from `reports.js`, `fmtDate`/`fmtMonth`) from the `jobReports`/`allEntries` props rather than the app's closure-bound `reportFigures`/`periodTitle` — keeps that computation in this already-lazy chunk instead of the eager budget. Tapping a row calls `onOpenReport`, opening the app's existing `reportViewModal` unchanged. Billing (new quote/invoice, this project's document list, Regie-as-quote/invoice, costing) moved out of Rapporte into a new owner-only `hubTab === "billing"` tab, spliced into the tab array only when `canBill` — a crew or supervisor account never renders the tab button at all. The Übersicht quick-action grid carries `[data-day-scan]` (`docs/specs/2026-09-14_ai-day-scan.md`), calling the app's `onOpenDayScan` — opens `roofing-site-manager.jsx`'s lazy `DayScanModal`. Below that grid, an owner/supervisor-only quick status control (`docs/specs/2026-09-15_row-actions-and-project-status.md`, `canManageStatus`/`onChangeStatus` props, `[data-project-status]`) offers a 5-status subset of `PROJECT_STATUSES` (now exported from `roofing-site-manager.jsx`) — `lead`/`quoted`/`construction`/`completed`/`hold`; `waiting`/`lost` stay reachable only via the project-edit form |
...
| `ui/day-scan-modal.jsx` | `DayScanModal` (`docs/specs/2026-09-14_ai-day-scan.md`): the in-app "log the day" flow, triggered by `[data-day-scan]` in `tabs/ProjectDetail.jsx`'s Übersicht tab. Lazy for the same reason `ReportSignModal` is — it owns its own Claude call (`callClaude`) and save logic (`persist`/`newEntry`, all passed in as props), not just render. State (`dayScanModal`) stays in `roofing-site-manager.jsx` so the app's own `toggleVoiceInput` can dictate into `state.text` the same way `ProjectDetail`'s chat composer already does. Capture step: a description (`[data-day-scan-text]`, mic via `[data-voice-note]`) and/or up to a few photos (`[data-day-scan-add-photo]`); `[data-day-scan-analyze]` calls `day-scan.js`'s prompt/parse pair. Review step: editable hours (`[data-day-scan-hours]`, optional — never pre-filled unless the model gave a real one), a checkable/editable item list (`[data-day-scan-item-check/-kind/-name/-qty/-unit]`, material or tool), a note (`[data-day-scan-note]`); `[data-day-scan-save]` builds one `newEntry()` each for a given kind and one `persist()` call, same batching `confirmScan` already does |
| `ui/customer-form.jsx` | `CustomerFormModal` (`docs/specs/2026-09-09_customer-fields.md`): the customer add/edit form, render only — `roofing-site-manager.jsx`'s `customerForm` state, `submitCustomer`/`deleteCustomer` stay put. Lazy: the full field set (`anrede`/`uidChe`/`role`/`phoneMobile`/`phoneOffice`/`lang`/`objectAddress`/`billingAddress`/`preferredChannel`/a repeating `contactPersons[]`/`source`, on top of the original five) missed the first-paint budget eager, the same shape `ui/trip-modal.jsx` and `ui/day-scan-modal.jsx` already hit — moved out once a field set gets this size |
...
| `customer-contact.js` | `customerContact(customer)` — the four fallback rules `docs/specs/2026-09-09_customer-fields.md` needs now that `phone`→`phoneMobile`/`phoneOffice` and `address`→`objectAddress`/`billingAddress` split without a migration: `{callNumber, whatsAppNumber, objectAddr, billingAddr}`, each new field first, the legacy one only when the new one is blank. `callNumber` may fall back to `phoneOffice`; `whatsAppNumber` never does |
...
| `day-scan.js` | `buildDayScanPrompt`, `parseDayScanResponse` — the in-app version: a photo and/or a typed/dictated description of a day, one `callClaude` call, turned into a validated `{hours, items, note}` plan for `ui/day-scan-modal.jsx`'s review step (`docs/specs/2026-09-14_ai-day-scan.md`). A malformed item is dropped rather than thrown on; `hours` is `null` unless the model gave a real positive number — never guessed |
| `rapport-import.js` | `normaliseImportRows`, `matchProjects` — pure planning for `scripts/import-rapport.mjs` (`docs/specs/2026-09-13_weekly-rapport-bulk-import.md`): validates a transcribed row and matches its project name (case-insensitive, exact) against existing projects, grouping unmatched names into a `toCreate` list. A row carries a `kind` (`time`/`material`/`tool`/`note`, default `time`); `time`/`material`/`tool` need a `qty`/`unit` pair (a `time` row may give `hours` instead, kept as a legacy alias — `docs/specs/2026-09-13_material-tool-bulk-import.md`), a `note` row needs neither — it becomes a plain chat note, `submitNote()`'s own shape (`docs/specs/2026-09-13_note-bulk-import.md`) |
...
| Suite | Covers |
|---|---|
| `logic.test.mjs` (~300) | every pure module, i18n completeness (every language has every `en.json` key, **and** every `t.*` key used in `tabs/`/`ui/` source exists in `en.json` itself — the hubBilling class of bug, where a key missing from `en.json` too was invisible to the first check), error codes ↔ docs, precache list, first-paint budget, palette contrast, this map's completeness |
...
- `scripts/stamp.mjs` (build stamp + `sw.js`), `scripts/seed-emulator.mjs`
  ...
  a transcribed weekly Rapport's rows, a material/tool list's, or chat
  notes, as real projects/entries, signed in as the owner with the client
  SDK — no admin SDK exists for this app; dry-run unless `--commit`;
