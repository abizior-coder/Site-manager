...
await check("crew CANNOT read finance", () => assertFails(getDoc(doc(crew, "companies", CID, "private", "finance"))));
await check("crew CANNOT read invoices", () => assertFails(getDoc(doc(crew, "companies", CID, "documents", "inv1"))));
...
await check("crew CAN read member list", () => assertSucceeds(getDoc(doc(crew, "companies", CID, "members", OWNER))));

// --- outsiders -----------------------------------------------------------
await check("outsider CANNOT read projects", () =>
  assertFails(getDoc(doc(outsider, "companies", CID, "projects", "p1"))),
);
await check("outsider CANNOT read company", () => assertFails(getDoc(doc(outsider, "companies", CID))));
...
await check("anonymous CANNOT read anything", () => assertFails(getDoc(doc(anon, "companies", CID, "projects", "p1"))));
...
await check("invite CANNOT be used to grant a higher role than it names", () =>
  assertFails(
    setDoc(doc(testEnv.authenticatedContext("u2").firestore(), "companies", CID, "members", "u2"), {
      role: "owner",
      inviteCode: "GOODCODE",
    }),
  ),
...
await check("expired invite is rejected", () =>
  assertFails(
    setDoc(doc(testEnv.authenticatedContext("u3").firestore(), "companies", CID, "members", "u3"), {
      role: "crew",
      inviteCode: "EXPIRED",
    }),
  ),
...
await check("made-up invite code is rejected", () =>
  assertFails(
    setDoc(doc(testEnv.authenticatedContext("u4").firestore(), "companies", CID, "members", "u4"), {
      role: "crew",
...
await testEnv.withSecurityRulesDisabled(async (ctx) => {
  ...
  await setDoc(doc(d, "companies", CID, "leave", "lv-crew"), {
    userId: CREW,
    date: "2026-09-02",
    status: "pending",
    type: "vacation",
  });
  await setDoc(doc(d, "companies", CID, "leave", "lv-own"), {
    userId: SUP,
    date: "2026-09-03",
    status: "pending",
    type: "vacation",
...
await check("supervisor CAN plan assignments", () =>
  assertSucceeds(
    setDoc(doc(sup, "companies", CID, "assignments", "a-sup"), { date: "2026-09-02", projectId: "p1", userId: CREW }),
  ),
);
...
await check("crew CAN request their own leave", () =>
  assertSucceeds(
    setDoc(doc(crew, "companies", CID, "leave", "lv-new"), {
      userId: CREW,
      date: "2026-09-05",
      status: "pending",
      type: "sick",
...
await check("crew CANNOT file leave for someone else", () =>
  assertFails(
    setDoc(doc(crew, "companies", CID, "leave", "lv-fake"), {
      userId: SUP,
      date: "2026-09-05",
      status: "pending",
      type: "sick",
...
await check("crew CANNOT approve their own leave", () =>
  assertFails(
    setDoc(doc(crew, "companies", CID, "leave", "lv-crew"), {
      userId: CREW,
      date: "2026-09-02",
      status: "approved",
      type: "vacation",
...
await check("supervisor CAN approve crew leave", () =>
  assertSucceeds(
    setDoc(doc(sup, "companies", CID, "leave", "lv-crew"), {
      userId: CREW,
      date: "2026-09-02",
      status: "approved",
      type: "sick",
...
await testEnv.withSecurityRulesDisabled(async (ctx) => {
  const d = ctx.firestore();
  await setDoc(doc(d, "companies", CID, "reports", "r-open"), {
    userId: CREW,
    projectId: "p1",
    date: "2026-09-01",
    signedAt: null,
  });
  await setDoc(doc(d, "companies", CID, "reports", "r-signed"), {
    userId: CREW,
    projectId: "p1",
    date: "2026-09-01",
    signedAt: Date.now(),
...
await testEnv.withSecurityRulesDisabled(async (ctx) => {
  const d = ctx.firestore();
  await setDoc(doc(d, "companies", CID, "files", "f-crew"), {
    uploadedBy: CREW,
    projectId: "p1",
    name: "Dach.pdf",
    kind: "plan",
  ...
  await setDoc(doc(d, "companies", CID, "files", "f-sup"), {
    uploadedBy: SUP,
    projectId: "p1",
    name: "Offerte.pdf",
    kind: "offer",
...
await check("crew CAN add a plan to a job", () =>
  assertSucceeds(
    setDoc(doc(crew, "companies", CID, "files", "f-new"), {
      uploadedBy: CREW,
      projectId: "p1",
      name: "Plan.pdf",
      kind: "plan",
...
await check("crew CANNOT add a plan in someone else's name", () =>
  assertFails(
    setDoc(doc(crew, "companies", CID, "files", "f-fake"), {
      uploadedBy: SUP,
      projectId: "p1",
      name: "Plan.pdf",
      kind: "plan",
...
await check("a stranger CANNOT see them", () => assertFails(getDoc(doc(stranger, "companies", CID, "files", "f-sup"))));
...
await check("a file link must be a web address", () =>
  assertFails(
    setDoc(doc(crew, "companies", CID, "files", "f-js"), {
      uploadedBy: CREW,
      projectId: "p1",
      name: "x",
      kind: "plan",
...
await check("a file may not claim to be html", () =>
  assertFails(
    setDoc(doc(crew, "companies", CID, "files", "f-html"), {
      uploadedBy: CREW,
      projectId: "p1",
      name: "Plan.pdf",
      kind: "plan",
...
await check("an https link is fine", () =>
  assertSucceeds(
    setDoc(doc(crew, "companies", CID, "files", "f-ok"), {
      uploadedBy: CREW,
      projectId: "p1",
      name: "Plan",
      kind: "plan",
      url: "https://example.ch/plan",
      createdAt: Date.now(),
    }),
  ),
);
...
await testEnv.withSecurityRulesDisabled(async (ctx) => {
  await setDoc(doc(ctx.firestore(), "companies", CID, "entries", "e-attr"), {
    userId: CREW,
    type: "time",
    qty: "8",
    date: "2026-09-02",
...
await check("crew CANNOT hand their own entry to someone else", () =>
  assertFails(
    setDoc(doc(crew, "companies", CID, "entries", "e-attr"), {
      userId: SUP,
      type: "time",
      qty: "8",
      date: "2026-09-02",
...
await check("a manager CANNOT re-attribute an entry either", () =>
  assertFails(
    setDoc(doc(sup, "companies", CID, "entries", "e-attr"), {
      userId: SUP,
      type: "time",
      qty: "8",
      date: "2026-09-02",
...
await check("crew CAN create a report on site", () =>
  assertSucceeds(
    setDoc(doc(crew, "companies", CID, "reports", "r-new"), {
      userId: CREW,
      projectId: "p1",
      date: "2026-09-02",
      signedAt: null,
...
await check("crew CANNOT create a report as someone else", () =>
  assertFails(
    setDoc(doc(crew, "companies", CID, "reports", "r-fake"), {
      userId: SUP,
      projectId: "p1",
      date: "2026-09-02",
      signedAt: null,
...
await check("manager CAN edit an unsigned report", () =>
  assertSucceeds(
    setDoc(doc(sup, "companies", CID, "reports", "r-open"), {
      userId: CREW,
      projectId: "p1",
      date: "2026-09-01",
      signedAt: null,
...
await testEnv.withSecurityRulesDisabled(async (ctx) => {
  const d = ctx.firestore();
  await setDoc(doc(d, "companies", CID, "kv", `site-docs-${OWNER}`), {
    value: JSON.stringify({ insurance: [{ provider: "CSS", policyNumber: "756.1234" }] }),
  });
  await setDoc(doc(d, "companies", CID, "kv", `site-profile-${SUP}`), {
    value: JSON.stringify({ name: "Polier", phone: "079", webhookUrl: "" }),
  });
  await setDoc(doc(d, "companies", CID, "kv", `clock-${SUP}`), {
    value: JSON.stringify({ projectId: "p1", startedAt: 1 }),
  });
  await setDoc(doc(d, "companies", CID, "kv", "photo-sig1"), {
...
await check("crew CANNOT overwrite a colleague's profile", () =>
  assertFails(
    setDoc(doc(crew, "companies", CID, "kv", `site-profile-${SUP}`), {
      value: JSON.stringify({ webhookUrl: "https://evil.example" }),
    }),
  ),
);
await check("crew CAN keep their own profile", () =>
  assertSucceeds(
    setDoc(doc(crew, "companies", CID, "kv", `site-profile-${CREW}`), { value: JSON.stringify({ name: "Hans" }) }),
  ),
);
await check("crew CANNOT rewrite a colleague's running clock", () =>
  assertFails(
    setDoc(doc(crew, "companies", CID, "kv", `clock-${SUP}`), {
      value: JSON.stringify({ projectId: "p1", startedAt: 0 }),
    }),
  ),
);
...
await check("a signature CANNOT be overwritten, not even by a manager", () =>
  assertFails(
    setDoc(doc(sup, "companies", CID, "kv", "photo-sig1"), {
...
await check("crew CAN add a photo they sign as theirs", () =>
  assertSucceeds(
    setDoc(doc(crew, "companies", CID, "kv", "photo-new1"), { value: "data:image/png;base64,AA==", by: CREW }),
  ),
);
await check("crew CANNOT add a photo in someone else's name", () =>
  assertFails(
    setDoc(doc(crew, "companies", CID, "kv", "photo-new2"), { value: "data:image/png;base64,AA==", by: SUP }),
  ),
);
...
await check("crew CAN learn a unit into the article master", () =>
  assertSucceeds(
    setDoc(doc(crew, "companies", CID, "kv", "site-material-catalog"), { value: '{"lattung":{"unit":"m"}}' }),
  ),
);
...
await check("crew CANNOT list the whole bucket", () => assertFails(getDocs(collection(crew, "companies", CID, "kv"))));
...
// --- offboarding --------------------------------------------------------------
await testEnv.withSecurityRulesDisabled(async (ctx) => {
  await setDoc(doc(ctx.firestore(), "companies", CID, "members", "u-gone"), {
    role: "crew",
    name: "Gone",
    email: "",
    active: false,
...
await testEnv.withSecurityRulesDisabled(async (ctx) => {
  const d = ctx.firestore();
  await setDoc(doc(d, "invites", "JOIN2026"), {
    companyId: CID,
    role: "crew",
    expiresAt: Date.now() + 86400000,
    createdAt: Date.now(),
  ...
  await setDoc(doc(d, "invites", "BURN2026"), {
    companyId: CID,
    role: "crew",
    expiresAt: Date.now() + 86400000,
    createdAt: Date.now(),
...
function joinBatch(db, uid, code) {
  ...
  b.set(doc(db, "companies", CID, "members", uid), {
    role: "crew",
    name: "New",
    email: "",
    active: true,
...
await check("joining WITHOUT consuming the code is refused", () =>
  assertFails(
    setDoc(doc(joiner2, "companies", CID, "members", "u-joiner2"), {
      role: "crew",
      name: "Sneak",
      email: "",
      active: true,
...
await check("a SIGNED report cannot be altered, even by the owner", () =>
  assertFails(
    setDoc(doc(owner, "companies", CID, "reports", "r-signed"), {
      userId: CREW,
      projectId: "p1",
      date: "2026-09-01",
      signedAt: Date.now(),
      signerName: "Jemand anders",
    }),
  ),
);
await check("crew CAN read reports", () => assertSucceeds(getDoc(doc(crew, "companies", CID, "reports", "r-signed"))));
...
await check("manager CAN correct a sent report", () =>
  assertSucceeds(
    setDoc(doc(sup, "companies", CID, "sentReports", "sr1"), { userId: CREW, period: "daily", note: "korrigiert" }),
  ),
);
...
await check("owner CAN create an assignment", () =>
  assertSucceeds(
    setDoc(doc(owner, "companies", CID, "assignments", "a1"), {
      date: "2026-09-01",
      projectId: "p1",
      userId: CREW,
    }),
...
await check("crew CANNOT reassign themselves", () =>
  assertFails(
    setDoc(doc(crew, "companies", CID, "assignments", "a1"), {
      date: "2026-09-01",
      projectId: "p-nicer",
      userId: CREW,
    }),
...
await check("a non-member CANNOT read the schedule", () =>
  assertFails(
    getDoc(doc(testEnv.authenticatedContext("stranger-uid").firestore(), "companies", CID, "assignments", "a1")),
  ),
);
...
await check("owner CAN create an invite", () =>
  assertSucceeds(
    setDoc(doc(owner, "invites", "NEWCODE"), {
      companyId: CID,
      role: "crew",
      expiresAt: Date.now() + 86400000,
      usedBy: null,
...
await check("owner CAN delete an invite", () => assertSucceeds(deleteDoc(doc(owner, "invites", "NEWCODE"))));
...
await check("old public local/* stays denied", () => assertFails(getDoc(doc(anon, "local", "site-data"))));
