...
export function Stat({ label, value, color }) {
  return (
    <div className="flex items-center justify-between py-1.5" style={{ borderBottom: `1px solid ${COLORS.border}` }}>
      ...
      <span style={{ color }} className="font-black text-lg">
        {value}
      </span>
    </div>
  );
...
export function EntryRow({ entry, projectName, t, onEditTime, onEditEntry, onDelete }) {
  ...
  return (
    ...
    >
      <div
        style={{ background: COLORS.shell, border: `1px solid ${meta.color}` }}
      ...
      <div className="flex-1 min-w-0">
        <div
          style={{ whiteSpace: entry.type === "inspection" ? "pre-wrap" : "normal" }}
        ...
        <div style={{ color: COLORS.muted }} className="text-xs mt-0.5">
          {meta.label}
          {entry.qty ? ` · ${entry.qty}${entry.unit ? " " + entry.unit : ""}` : ""}
          {entry.projectId ? ` · ${projectName(entry.projectId)}` : ""}
        </div>
    ...
    <div className="flex flex-col gap-2">
      {presentTypes.map((ty) => {
        ...
        return (
          ...
          >
            <button
              onClick={() => setExpanded((s) => ({ ...s, [ty]: !s[ty] }))}
              className="w-full flex items-center justify-between px-3 py-2.5"
            >
              <span className="flex items-center gap-2 text-sm font-semibold">
                <Icon size={15} color={meta.color} /> {meta.label}
                <span style={{ color: COLORS.muted }} className="text-xs font-normal">
                  ({items.length})
                </span>
              </span>
              <ChevronRight
