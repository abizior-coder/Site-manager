...
import { handleErrors, cleanReport, appendError, summarise, kvKey, MAX_PER_DAY, RETENTION_SECONDS } from "./src/errors.js";
...
function fakeKv() {
  const store = new Map(), ttls = new Map();
  return { store, ttls, async get(k) { return store.has(k) ? store.get(k) : null; }, async put(k, v, o) { store.set(k, v); ttls.set(k, o && o.expirationTtl); } };
}
...
t("a report keeps only the listed fields, cut to size", cleanReport({ build: "8f1ef53cc8", code: "E91", tag: "CRASH", message: "x".repeat(300), stack: "  at a\n  at b", uid: "secret", ua: "ios" }), { build: "8f1ef53cc8", code: "E91", tag: "CRASH", message: "x".repeat(200), stack: "at a at b", ua: "ios" });
t("nothing without message or code", cleanReport({ build: "abc" }), null);
{
  ...
  t("the day counts every report but keeps at most the cap", [day.count, day.items.length, day.items[0].message], [MAX_PER_DAY + 5, MAX_PER_DAY, "m5"]);
}
t("the summary has a count per day and the newest ten", (() => { const s = summarise([{ date: "2026-09-04", day: { count: 2, items: [{ at: 1, message: "a" }, { at: 3, message: "c" }] } }, { date: "2026-09-05", day: { count: 1, items: [{ at: 2, message: "b" }] } }]); return [s.days, s.recent.map((x) => x.message)]; })(), [[{ date: "2026-09-04", count: 2 }, { date: "2026-09-05", count: 1 }], ["c", "b", "a"]]);

const env = { RATE_LIMIT: fakeKv() };
const report = { build: "8f1ef53cc8", code: "E91", tag: "CRASH", message: "Cannot read properties of undefined", stack: "TypeError: …", path: "/index.html", lang: "de", ua: "android" };
...
t("the day record holds the cleaned report with the time", JSON.parse(env.RATE_LIMIT.store.get(kvKey("c1", "2026-09-05"))).items.map((x) => [x.at, x.code, x.ua]), [[NOW, "E91", "android"]]);
...
t("crew cannot read the log", (await call(env, "GET", "/errors/c1?days=7", "tok-crew")).status, 403);
{
  ...
  t("the owner reads seven days and the recent list", [r.status, r.body.days.length, r.body.days[6], r.body.recent[0].message], [200, 7, { date: "2026-09-05", count: 1 }, "Cannot read properties of undefined"]);
}
