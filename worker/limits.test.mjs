...
}
{
  ...
  t("the two-hundred-and-first is not", (await checkLimits(kv, { uid: "u1", cid: "c1" }, T0 + ACCOUNT_DAILY_LIMIT * 61000)).status, 429);
  t("a fresh account in the same company still has its own daily cap", await checkLimits(kv, { uid: "u2", cid: "c1" }, T0 + 1000), null);
}
{
  const kv = fakeKv();
  ...
  let n = 0, last = null;
  for (let i = 0; i < COMPANY_DAILY_LIMIT; i++) { last = await checkLimits(kv, { uid: uids[i % 4], cid: "c1" }, T0 + i * 61000); n++; }
