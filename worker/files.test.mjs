...
function fakeBucket() {
  ...
  return {
    store,
    async put(key, body, opts) { store.set(key, { body: new Uint8Array(body), size: body.byteLength, httpMetadata: opts.httpMetadata, customMetadata: opts.customMetadata }); },
    async get(key) { const o = store.get(key); return o ? { body: o.body, size: o.size, httpMetadata: o.httpMetadata, customMetadata: o.customMetadata } : null; },
    async head(key) { const o = store.get(key); return o ? { size: o.size, customMetadata: o.customMetadata } : null; },
...
function call(env, method, path, { token, form, body } = {}) {
  ...
  return handleFiles({ request: new Request(`https://w.example${path}`, init), env, headers: { "Access-Control-Allow-Origin": "*" }, verify, isMember });
}
...
const env = { FILES: fakeBucket() };
{
  ...
  t("the upload answers with the file's identity", [meta.name, meta.type, meta.kind, meta.projectId, meta.uploadedBy], ["Dach Süd.pdf", "application/pdf", "plan", "p1", "u-crew"]);
...
// --- limits and refusals ----------------------------------------------------------
{
  ...
  const exe = await call(env, "POST", "/files/c1/p1", { token: "tok-crew", form: pdfForm("setup.exe", 10, "other", "application/octet-stream") });
  t("an executable is refused", exe.status, 415);
  const noFile = await call(env, "POST", "/files/c1/p1", { token: "tok-crew", form: (() => { const f = new FormData(); f.append("kind", "plan"); return f; })() });
