...
// --- pure ------------------------------------------------------------------
t("a batch folds into the day", mergeDay(null, { events: { "entry.material": 2, open: 1 }, uidHash: "abc" }), {
  events: { "entry.material": 2, open: 1 },
  users: { abc: 1 },
});
t(
  "a second person adds up",
  mergeDay({ events: { open: 1 }, users: { abc: 1 } }, { events: { open: 1 }, uidHash: "def" }),
  { events: { open: 2 }, users: { abc: 1, def: 1 } },
);
t(
  "bad names and zero counts are dropped",
  mergeDay(null, { events: { "Bad Name": 3, ok: 0, fine: 1 }, uidHash: "x" }),
  { events: { fine: 1 }, users: { x: 1 } },
);
t("a count is capped per post", mergeDay(null, { events: { open: 999999 } }).events.open, MAX_COUNT_PER_EVENT);
t(
  "the summary counts people once across days",
  summarise([
    { date: "d1", day: { events: { open: 2 }, users: { a: 1, b: 1 } } },
    { date: "d2", day: { events: { open: 1, trip: 1 }, users: { a: 1 } } },
    { date: "d3", day: null },
  ]),
  {
    days: [
      { date: "d1", active: 2, events: { open: 2 } },
      { date: "d2", active: 1, events: { open: 1, trip: 1 } },
      { date: "d3", active: 0, events: {} },
    ],
    totals: { open: 3, trip: 1 },
    activePeople: 2,
  },
);
t(
  "the uid hash is short and stable",
  [
    (await hashUid("u-chef")).length,
    (await hashUid("u-chef")) === (await hashUid("u-chef")),
    (await hashUid("u-chef")) === (await hashUid("u-crew")),
  ],
  [12, true, false],
);
...
// --- routes ----------------------------------------------------------------
{
  ...
  t(
    ...
    (await call(env, "POST", "/metrics/c1", "tok-other", { events: { open: 1 } })).status,
    403,
  );
  t(
    "a crew member posts counts",
    (await call(env, "POST", "/metrics/c1", "tok-crew", { events: { open: 1, "entry.time": 3 } })).body,
    { ok: true },
  );
  ...
  t(
    "the day holds counts and two short hashes, no uids",
    [stored.events, Object.keys(stored.users).length, Object.keys(stored.users).some((h) => h.includes("u-"))],
    [{ open: 2, "entry.time": 3 }, 2, false],
  );
  ...
  t(
    "the owner reads seven days ending today",
    [r.status, r.body.days.length, r.body.days[6].date, r.body.days[6].active, r.body.totals, r.body.activePeople],
    [200, 7, "2026-09-03", 2, { open: 2, "entry.time": 3 }, 2],
  );
  t(
    "another company's owner sees nothing of c1",
    (await call(env, "GET", "/metrics/c1?days=7", "tok-other")).status,
    403,
  );
  t("a bad company id is refused", (await call(env, "GET", "/metrics/../x", "tok-chef")).status, 404);
  t(
    "without KV the route says so",
    (await call({}, "POST", "/metrics/c1", "tok-crew", { events: { open: 1 } })).status,
    503,
  );
}
