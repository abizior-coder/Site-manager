...
export function ProjectDetail({
  ...
  const chatSeen = (() => {
    try {
      return seenKey ? parseInt(localStorage.getItem(seenKey) || "0", 10) || 0 : 0;
    } catch {
      return 0;
    }
  })();
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {project.category && (
                <span
                  style={{ background: COLORS.cardAlt, color: COLORS.muted, border: `1px solid ${COLORS.border}` }}
            ...
            {project.address ? (
              ...
              <button
                ...
                className="text-xs flex items-center gap-1 mt-0.5 underline"
              >
          ...
          <div className="flex items-center gap-3 shrink-0 pt-1">
            ...
            <button
              ...
              title={t.a11yShare}
              onClick={() => onShare(project, entries)}
              style={{ color: COLORS.muted }}
            >
              <Share2 size={16} />
            </button>
        ...
        <div className="lg:max-w-4xl lg:mx-auto">
            ...
            {[
              ...
              return (
                <button
                  ...
                  className="shrink-0 px-3 py-1.5 rounded-full text-xs font-bold uppercase flex items-center gap-1.5"
                >
                  {label}
                  {count > 0 && (
                    <span
                      data-hub-count={id}
                      style={{
                        background: on ? "#ffffff33" : id === "chat" ? COLORS.accent : COLORS.cardAlt,
                        color: "#fff",
                      }}
          ...
          {hubTab === "overview" && (
            ...
            the worker opens it and taps. */}
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                  ...
                  >
                    <Package size={13} color={COLORS.success} /> {t.materials}
                  </button>
                  <button
                    onClick={() => onAdd("tool")}
                    style={{ background: COLORS.card, border: `1px solid ${COLORS.border}` }}
                    className="py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-1"
                  >
                    <Wrench size={13} color={COLORS.amber} /> {t.tools}
                  </button>
                  <button
                    onClick={() => onAdd("photo")}
                    style={{ background: COLORS.card, border: `1px solid ${COLORS.border}` }}
                    className="py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-1"
                  >
                    <Camera size={13} color="#7FA0C7" /> {t.photoLabel}
                  </button>
                  <button
                    onClick={() => onScanCompare(project.id)}
                  ...
                  <button
                    ...
                    className="col-span-2 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-1"
                  >
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      ...
                      {customer.email && (
                        <a
                          ...
                          style={{ background: COLORS.cardAlt, color: "#B48EAD" }}
                          className="px-2 py-1 rounded text-xs font-bold flex items-center gap-1"
                        >
            ...
            HTML5 drag does not exist. Both do the same thing. */}
                ...
                >
                  <div style={{ color: COLORS.muted }} className="text-xs uppercase tracking-wide mb-2">
                    {t.crewOnJob} ({onCrew.length})
                  </div>
                  {onCrew.length === 0 ? (
                    <div style={{ color: COLORS.muted }} className="text-xs mb-2">
                      {canManageCrew ? t.crewDropHint : t.crewNobody}
                    </div>
                  ) : (
              ...
              <div
                onDragOver={(e) => {
                  if (Array.from(e.dataTransfer?.types || []).includes("Files")) {
                    e.preventDefault();
                    if (!filesOver) setFilesOver(true);
                  }
                }}
                onDragLeave={(e) => {
                  if (!e.currentTarget.contains(e.relatedTarget)) setFilesOver(false);
                }}
                onDrop={(e) => {
                ...
                style={{
                  background: filesOver ? `${COLORS.accent}1A` : COLORS.card,
                  border: `1px ${filesOver ? "dashed" : "solid"} ${filesOver ? COLORS.accent : COLORS.border}`,
                }}
                className="rounded-xl p-3 mb-3"
              >
                <div className="flex items-center justify-between gap-2 mb-2 flex-wrap">
                  <div
                    style={{ color: COLORS.muted }}
                    className="text-xs uppercase tracking-wide flex items-center gap-1.5 min-w-0"
                  >
                    <FileText size={11} />{" "}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <ExternalLink size={11} /> {t.filesAddLink}
                    </button>
                    <button
                ...
                {(files || []).length === 0 ? (
                  <div style={{ color: COLORS.muted }} className="text-xs">
                    {filesOver ? t.filesDropHint : t.filesEmpty}
                  </div>
                ) : (
                    ...
                    {sortFiles(files).map((f) => {
                      ...
                      return (
                        ...
                        >
                          <Icon
                            size={15}
                            color={f.kind === "plan" ? COLORS.accent : COLORS.muted}
                            className="shrink-0"
                          />
                          <button onClick={() => onOpenFile(f)} className="flex-1 min-w-0 text-left">
                            ...
                            <div style={{ color: COLORS.muted }} className="text-xs truncate">
                              {[
                                t[`fileKind_${f.kind}`] || f.kind,
                                f.url ? t.filesLinkLabel : fmtSize(f.size),
              ...
              {reports && reports.length > 0 && (
                <div className="mb-4 flex flex-col gap-1.5">
                  {reports.map((r) => (
                    <button
                      key={r.id}
                      onClick={() => onPrintRapport(r)}
                      style={{ background: COLORS.card }}
                      className="w-full text-left rounded-lg px-3 py-2 flex items-center justify-between gap-2"
                    >
                      <div className="min-w-0">
                        <div className="text-sm truncate">
                          {t.rapportTitle} · {r.date}
                        </div>
                        <div style={{ color: COLORS.muted }} className="text-xs truncate">
                          {r.signerName} · {r.hours} h
                          {rapportChanged(
                            r,
                            entries.filter((e) => e.date === r.date),
                          ) && <span style={{ color: COLORS.amber }}> · {t.rapportChangedSince}</span>}
                        </div>
                      </div>
                    ...
                    return (
                      ...
                      >
                        <button onClick={() => onOpenDocument(d)} className="flex-1 min-w-0 text-left">
                          ...
                          <div style={{ color: COLORS.muted }} className="text-xs">
                            {d.date} · {st.totals.gross.toFixed(2)}
                          </div>
                        </button>
                        <span
                          style={{
                        ...
                        <button
                          aria-label={t.a11yPrint}
                          title={t.a11yPrint}
                          onClick={() => onPrintDocument(d)}
                          style={{ color: COLORS.muted }}
                          className="tap shrink-0"
                        >
                          <Printer size={14} />
              ...
              {regie && regie.count > 0 && (
                ...
                >
                  ...
                  <div className="flex flex-col gap-1 text-sm">
                    ...
                    {canBill && (
                      <div
                        style={{ borderTop: `1px solid ${COLORS.amber}33` }}
                  ...
                  {canBill && regie.unpriced > 0 && (
                    <div style={{ color: COLORS.amber }} className="text-xs mt-1.5">
                      {regie.unpriced} {t.costingUnpriced}
                    </div>
                  )}
                  {canBill && (
                    <div className="grid grid-cols-2 gap-2 mt-3">
                      <button
                        onClick={() => onRegieDocument("quote")}
                        style={{ background: COLORS.cardAlt, border: `1px solid ${COLORS.border}` }}
                      ...
                      <button
                        onClick={() => onRegieDocument("invoice")}
                        style={{ background: COLORS.amber, color: "#241C00" }}
                        className="py-2 rounded-lg text-xs font-bold uppercase"
                      >
                        {t.regieAsInvoice}
                      </button>
                ...
                >
                  ...
                  <div className="flex flex-col gap-1 text-sm">
                    <div className="flex justify-between">
                      <span style={{ color: COLORS.muted }}>
                        {t.labourCost} ({costing.hours.toFixed(1)} h)
                      </span>
                  ...
                  {(!costing.hasRate || costing.unpricedCount > 0) && (
                    <div style={{ color: COLORS.amber }} className="text-xs mt-2 leading-relaxed">
                      ...
                      {costing.unpricedCount > 0 && (
                        <div>
                          {costing.unpricedCount} {t.costingUnpriced}
                        </div>
                      )}
                    </div>
                  )}
              ...
              {(() => {
                ...
                if (used.length <= 1) {
                  return (
                    ...
                    >
                      <Section
                        title={`${t.materials} (${materials.length})`}
                        items={materials}
                        onEditItem={onEditEntry}
                        onCopyItem={onCopyEntry}
                        onDeleteItem={onDeleteEntry}
                ...
                return used.map((tr) => {
                  ...
                  return (
                    <div
                      key={tr.key}
                      style={{ background: COLORS.card, border: `1px solid ${tr.color}55` }}
                      className="rounded-xl p-4 mb-4"
                    >
                      <div className="flex items-center justify-between gap-2 mb-3">
                        <div className="flex items-center gap-2">
                          <span style={{ background: tr.color }} className="w-2.5 h-2.5 rounded-full shrink-0" />
                          <span style={{ color: tr.color }} className="text-xs font-black uppercase tracking-wide">
                            {t[tr.labelKey]}
                          </span>
                        </div>
                        {tradeHours > 0 && (
                          <span style={{ color: COLORS.muted }} className="text-xs font-bold">
                            {tradeHours.toFixed(1)} h
                          </span>
                        )}
                      </div>
                      {mine.length > 0 && (
                        <Section
                          title={`${t.materials} (${mine.length})`}
                          items={mine}
                          onEditItem={onEditEntry}
                          onCopyItem={onCopyEntry}
                          onDeleteItem={onDeleteEntry}
          ...
          {hubTab === "overview" && (
            ...
            job -- both used to live only on Today. */}
              {inspections.length > 0 && (
                <div
                  data-job-inspections
                  style={{ background: COLORS.card, border: `1px solid #6FB3D955` }}
                  className="rounded-xl p-4 mb-4"
                >
                  <div
                    style={{ color: "#6FB3D9" }}
                    className="text-xs font-black uppercase tracking-wide mb-2 flex items-center gap-1.5"
                  >
                    <ClipboardCheck size={13} /> {t.jobInspections} ({inspections.length})
                  </div>
                    ...
                    {inspections.map((e) => (
                      ...
                      >
                        <div className="flex items-center gap-2 text-xs" style={{ color: COLORS.muted }}>
                          ...
                          {e.wasteKg > 0 && (
                            <span style={{ color: COLORS.amber }} className="ml-auto font-bold">
                              {t.inspectWaste}: ~{e.wasteKg} kg
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex gap-2 justify-end">
                          {canEditInspection && canEditInspection(e) && (
                            <button
                              className="tap"
                              aria-label={t.a11yEdit}
                              data-inspect-edit
                              onClick={() => onEditInspection(e)}
                              title={t.editLabel}
                              style={{ color: COLORS.muted }}
                            >
                              <Pencil size={13} />
              ...
              {trips.length > 0 && (
                <div
                  data-job-trips
                  style={{ background: COLORS.card, border: `1px solid #C68B4F55` }}
                  className="rounded-xl p-4 mb-4"
                >
                  <div
                    style={{ color: "#C68B4F" }}
                    className="text-xs font-black uppercase tracking-wide mb-2 flex items-center gap-1.5"
                  >
                    <Truck size={13} /> {t.jobTrips} ({trips.length})
                  </div>
                  <div className="flex flex-col gap-1.5">
                    {trips.map((e) => (
                      <div
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {pickFor === n.id && missing.length > 0 && (
                              <div data-translate-pick className="flex flex-wrap items-center gap-1.5 mt-2">
                                <span style={{ color: COLORS.muted }} className="text-xs uppercase tracking-wide">
                                  {t.translateInto}
                                </span>
                                {missing.map((c) => (
                                  <button
              ...
              <div className="flex gap-2 mt-3">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <button
                    ...
                    style={{
                      background: noteDraft.trim() ? COLORS.accent : COLORS.cardAlt,
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {photos.map((p) => (
                      <div key={p.id} className="relative">
                        ...
                        {p.originalPhotoId && (
                          <span
                            style={{ background: "rgba(0,0,0,0.65)", color: COLORS.amber }}
                            className="absolute bottom-1 left-1 px-1 rounded text-xs font-bold uppercase"
                          >
                            {t.photoEditedTag}
                          </span>
                        )}
          ...
          {hubTab === "time" &&
            (() => {
              ...
              const rows = entries
                .filter((e) => e.type === "time" || e.type === "break")
              ...
              const hoursOf = (list) =>
                Math.round(
                  list.reduce((s, e) => s + (parseFloat(e.qty) || 0) * (e.type === "break" ? -1 : 1), 0) * 100,
                ) / 100;
              return (
                <div
                  data-hub-time
                  ...
                  {Object.keys(byDay)
                    ...
                    .map((day) => (
                      <div key={day} className="mb-2">
                        ...
                        >
                          <span style={{ color: COLORS.muted }}>{day}</span>
                          <span className="font-bold">{hoursOf(byDay[day]).toFixed(1)} h</span>
                        </div>
                        {byDay[day].map((e) => (
                          <div key={e.id} className="flex items-center justify-between text-xs py-1">
                            <span>
                              {memberName(e.userId) || "—"}
                              {e.type === "break" ? ` · ${t.typeBreak}` : ""}
                            </span>
                            <span style={{ color: COLORS.muted }} className="tabular-nums">
                              {e.startTime ? `${e.startTime}–${e.endTime || ""} · ` : ""}
                              {e.type === "break" ? "−" : ""}
                              {(parseFloat(e.qty) || 0).toFixed(2)} h
                            </span>
                          </div>
                        ))}
                      </div>
  ...
  const onPointerDown = (e) => {
    ...
    if (pointers.current.size === 1) {
      ...
      if (now - lastTap.current < 300) {
        zoomAt(view.scale > 1 ? 0 : 2.5, e.clientX, e.clientY);
        lastTap.current = 0;
        return;
      }
      lastTap.current = now;
    ...
    >
      ...
      >
        <div className="min-w-0">
          ...
          <div style={{ color: COLORS.muted }} className="text-xs">
            {entry?.date}
            {entry?.originalPhotoId ? ` · ${t.photoEditedTag}` : ""} · {Math.round(view.scale * 100)}%
          </div>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button
            aria-label={t.a11yAction}
            onClick={() => zoomAt(0.7, window.innerWidth / 2, window.innerHeight / 2)}
            title={t.photoZoomOut}
            style={{ color: COLORS.muted }}
            className="tap w-9 h-9 flex items-center justify-center"
          >
            <ZoomOut size={18} />
          </button>
          <button
          ...
          <a
            href={src}
            download={`${(entry?.description || "foto").replace(/[^\w.-]+/g, "_")}.jpg`}
      ...
      >
        <img
          ...
          style={{
            ...
            transition: gesture.current ? "none" : "transform 0.08s",
            willChange: "transform",
          }}
        />
      </div>
      <div style={{ color: COLORS.muted }} className="text-xs text-center py-1.5 shrink-0">
        {t.photoZoomHint}
      </div>
    </div>
  );
...
export function PhotoEditor({ src, onCancel, onSave, t }) {
  ...
  const drawShape = (ctx, c, sh) => {
    ...
    if (sh.tool === "pen" && sh.points.length) {
      ...
      ctx.strokeRect(
        Math.min(sh.from.x, sh.to.x),
        Math.min(sh.from.y, sh.to.y),
        Math.abs(sh.to.x - sh.from.x),
        Math.abs(sh.to.y - sh.from.y),
      );
    } else if (sh.tool === "circle") {
      const rx = Math.abs(sh.to.x - sh.from.x) / 2,
        ry = Math.abs(sh.to.y - sh.from.y) / 2;
      ctx.beginPath();
      ctx.ellipse(
        (sh.from.x + sh.to.x) / 2,
        (sh.from.y + sh.to.y) / 2,
      ...
      ctx.lineWidth = Math.max(2, sh.width / 2);
      ctx.strokeStyle = "rgba(0,0,0,0.85)";
      ctx.strokeText(sh.text, sh.at.x, sh.at.y);
      ctx.fillText(sh.text, sh.at.x, sh.at.y);
    }
    ctx.restore();
  };
  ...
  return (
    ...
    >
      <div
        ...
        className="flex items-center justify-between gap-2 px-3 py-2 shrink-0"
      >
        <button onClick={onCancel} style={{ color: COLORS.muted }} className="text-xs font-bold uppercase">
          {t.back}
        </button>
        <div className="flex items-center gap-1">
          <button
            ...
            title={t.photoUndo}
            style={{ color: shapes.length ? COLORS.text : COLORS.border }}
            className="tap w-9 h-9 flex items-center justify-center"
          >
            <Undo2 size={18} />
          </button>
          <button
            ...
            onClick={save}
            disabled={!ready}
            style={{ background: COLORS.accent, opacity: ready ? 1 : 0.5 }}
            className="px-4 h-9 rounded-lg text-xs font-bold uppercase"
          >
            {t.saveLabel}
          </button>
        </div>
      ...
      {textAt && (
        ...
        >
          ...
          <button
            onClick={commitText}
            style={{ background: COLORS.accent }}
            className="px-3 h-9 rounded-lg text-xs font-bold uppercase"
          >
            OK
          </button>
        </div>
      ...
      <div
        style={{ background: COLORS.card, borderTop: `1px solid ${COLORS.border}` }}
        className="flex items-center justify-between gap-2 px-3 py-2 shrink-0 flex-wrap"
      >
        <div className="flex items-center gap-1">
          {tools.map(([key, Icon]) => (
            <button
        ...
        <div className="flex items-center gap-1.5">
          {PHOTO_COLOURS.map((c) => (
            <button
              key={c}
              onClick={() => setColour(c)}
              style={{ background: c, outline: colour === c ? `2px solid ${COLORS.text}` : "none", outlineOffset: 2 }}
        ...
        <div className="flex items-center gap-1">
          {[1, 2, 3].map((n) => (
            ...
            >
              <span
                style={{ background: "currentColor", width: 4 + n * 4, height: 4 + n * 4 }}
