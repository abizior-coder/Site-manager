...
const HEADERS = {
  ...
  phone: ["telefon", "phone", "tel", "tel.", "telefon fix", "telefon mobil", "mobile", "mobil", "natel", "t\u00e9l\u00e9phone", "telefono", "handy"],
...
export function parseCustomersCsv(text) {
  ...
  const cols = {};
  splitRow(lines[0], delim).forEach((h, i) => { const f = headerField(h); if (f) (cols[f] = cols[f] || []).push(i); });
  if (!cols.name && !cols.company && !cols.cpLast) return { rows: [], warnings: ["no-name-column"] };
  const rows = [];
  let dropped = 0;
  for (const line of lines.slice(1)) {
    const cells = splitRow(line, delim);
    const get = (f) => { for (const i of cols[f] || []) { const v = String(cells[i] || "").trim(); if (v) return v; } return ""; };
    ...
    let name = person;
    if (FIRMA.includes(kind)) { company = company || get("name"); name = contact; }
    ...
    rows.push({ name, company, phone: get("phone") || get("cpPhone"), email: get("email") || get("cpEmail"), address, notes: get("notes") });
  }
...
export function mergeCustomers(existing, rows) {
  ...
  const added = [], skipped = [];
  for (const r of rows || []) {
    const e = norm(r.email), k = `${norm(r.name)}|${norm(r.company)}`;
    if ((e && byEmail.has(e)) || byName.has(k)) { skipped.push(r); continue; }
    if (e) byEmail.add(e);
    byName.add(k);
    added.push({ id: uid(), name: r.name, company: r.company || "", phone: r.phone || "", email: r.email || "", address: r.address || "", notes: r.notes || "", contacts: [], createdAt: Date.now() });
  }
