...
const isWeekday = (d) => { const wd = new Date(`${d}T00:00:00Z`).getUTCDay(); return wd >= 1 && wd <= 5; };
...
const customerLabel = (customers, id) => { const c = (customers || []).find((x) => x.id === id) || {}; return (c.company || c.name || "").trim(); };
...
export const JOURNAL_HEADERS = ["Rechnungs-Nr", "Datum", "Fällig", "Kunde", "Baustelle", "Netto", "MWST %", "MWST", "Brutto", "Bezahlt", "Offen", "Status", "Aus Offerte"];
...
export function invoiceJournal(documents, customers, projects, month, today = `${month}-31`) {
  return monthInvoices(documents, month).map((d) => {
    ...
    return [d.number || "", d.date || "", d.dueDate || "", customerLabel(customers, d.customerId), projectLabel(projects, d.projectId),
      num(st.totals.net), num(st.totals.rate), num(st.totals.vat), num(st.totals.gross), num(st.paid), num(st.outstanding), STATUS_DE[st.key] || st.key, d.fromQuote || ""];
  });
}

export const POSITION_HEADERS = ["Rechnungs-Nr", "Datum", "Kunde", "Pos", "Beschreibung", "Menge", "Einheit", "Einzelpreis", "Betrag"];
...
export function invoicePositions(documents, customers, month) {
  ...
  for (const d of monthInvoices(documents, month)) {
    (d.lineItems || []).forEach((li, idx) => {
      const qty = parseFloat(li.qty || 0) || 0, price = parseFloat(li.unitPrice || 0) || 0;
      rows.push([d.number || "", d.date || "", customerLabel(customers, d.customerId), idx + 1, li.description || "", num(qty), li.unit || "", num(price), num(qty * price)]);
    });
  }
  return rows;
}

export const PAYROLL_HEADERS = ["Mitarbeiter", "E-Mail", "Arbeitstage", "Normal", "Überstunden", "Reisezeit", "Pausen", "Netto", "Soll", "Differenz", "Ferien", "Krank", "Andere Absenzen"];

const approvedLeave = (leaveRequests, uid, month) =>
  (leaveRequests || []).filter((l) => l.userId === uid && l.status === "approved" && String(l.date || "").startsWith(month));
...
export function payrollRows(entries, members, month, weeklyHours, leaveRequests) {
  ...
  return (members || []).map((m) => {
    ...
    const perDay = days.map((d) => splitDayHours(mine.filter((e) => e.date === d), contractDaily));
    const sum = (k) => r2(perDay.reduce((s, x) => s + x[k], 0));
    const leave = approvedLeave(leaveRequests, m.uid, month);
    const count = (type) => leave.filter((l) => (type ? l.type === type : !["vacation", "sick"].includes(l.type))).length;
    ...
    const net = sum("net");
    return {
      uid: m.uid, name: m.name || m.email || m.uid, email: m.email || "",
      worked: perDay.filter((x) => x.net > 0).length,
      normal: sum("normal"), overtime: sum("overtime"), travel: sum("travel"), breaks: sum("breaks"), net,
      target, diff: target == null ? null : r2(net - target),
      vacation: count("vacation"), sick: count("sick"), other: count(null),
    };
  });
}

export function payrollCsv(rows) {
  return toCsv(PAYROLL_HEADERS, rows.map((r) => [r.name, r.email, r.worked, num(r.normal), num(r.overtime), num(r.travel), num(r.breaks), num(r.net), r.target == null ? "" : num(r.target), r.diff == null ? "" : num(r.diff), r.vacation, r.sick, r.other]));
}

export const PAYROLL_DAY_HEADERS = ["Mitarbeiter", "E-Mail", "Datum", "Normal", "Überstunden", "Reisezeit", "Pausen", "Netto", "Baustellen"];
...
export function payrollDays(entries, members, projects, month, weeklyHours) {
  ...
  for (const m of members || []) {
    ...
    for (const d of monthDays(month)) {
      ...
      rows.push([m.name || m.email || m.uid, m.email || "", d, num(h.normal), num(h.overtime), num(h.travel), num(h.breaks), num(h.net), sites]);
    }
  }
  return rows;
}

export const CONTACT_HEADERS = ["Kontaktart", "Name", "Vorname", "Adresse", "PLZ", "Ort", "Land", "Telefon", "E-Mail", "Kontaktperson 1 Nachname", "Kontaktperson 1 Vorname"];
...
export function contactRows(customers) {
  return (customers || []).map((c) => {
    ...
    if (firma) return ["Firma", firma, "", a.street, a.postalCode, a.town, "CH", c.phone || "", c.email || "", person, ""];
