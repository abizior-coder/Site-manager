...
function fakeDt() {
  ...
  return {
    ...
    setData(k, v) {
      data[k] = v;
      if (!this.types.includes(k)) this.types.push(k);
    },
    getData(k) {
      return data[k] || "";
    },
...
check(
  "dock shows the active job on its own",
  dockText().includes("Trockenbau") && !dockText().includes("Dach Kontrolle") && !dockText().includes("Fertig"),
  dockText().slice(0, 120),
);
...
await click(
  btns().find((b) => (b.textContent || "").includes("Dach Kontrolle")),
  300,
);
...
check(
  "pinned job appears as a dock tile, first",
  !!tile() && (tile().textContent || "").includes("Dach Kontrolle"),
  "pinned lead is not the first tile: " + dockText().slice(0, 120),
);
...
check(
  "dropping a chip books material on the job",
  matsAfter === matsBefore + 1,
  `tile count ${matsBefore} -> ${matsAfter}`,
);
check(
  "the drop is confirmed by name",
  txt().includes(`${(chip?.textContent || "").trim()} → ${tileName}`),
  "no toast naming the job",
);

// --- carry a person onto it -----------------------------------------------------
await click(btns().find((b) => (b.textContent || "").trim() === "Team"));
const card = [...window.document.querySelectorAll("[draggable='true']")].find((el) =>
  (el.textContent || "").includes("Mitarbeiter"),
);
...
window.document
  ...
  ?.dispatchEvent(new window.MouseEvent("click", { bubbles: true })); // materials sit on the hub's Material tab
await new Promise((r) => setTimeout(r, 250));
check(
  "the job now lists the dropped material",
  txt().includes((chip?.textContent || "").trim()),
  "dropped item missing from the job view",
);
...
// --- search: one box, every hit draggable -------------------------------------
{
  ...
  if (box) {
    ...
    check(
      "search hides the category browser",
      !btns().some((b) => (b.textContent || "").trim() === "Holz"),
      "category buttons still shown while searching",
    );
    set.call(box, "");
    box.dispatchEvent(new window.Event("input", { bubbles: true }));
    await new Promise((r) => setTimeout(r, 150));
    check(
      "clearing the search brings the browser back",
      btns().some((b) => (b.textContent || "").trim() === "Holz"),
      "categories missing after clearing",
    );
  }
}
...
// --- a project card can be dragged onto the tray -----------------------------------
{
  ...
  const card = [...window.document.querySelectorAll("[draggable='true']")].find((el) =>
    (el.textContent || "").includes("Dach Kontrolle"),
  );
...
// --- one tap cycles the order --------------------------------------------------------
{
  ...
  await click(sortBtn(), 150);
  check(
    "four taps come back round",
    (sortBtn()?.textContent || "").trim() === before,
    (sortBtn()?.textContent || "").trim(),
  );
}
